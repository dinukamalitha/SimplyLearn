...
import { Save, CheckCircle, Upload, AlertCircle, File, Download, Clock } from 'lucide-react';

const AssignmentHeader = ({ assignment, id, isOverdue, deadline, submission }) => (
    <div className="p-8 mb-8 border bg-white/5 border-white/10 rounded-2xl">
        ...
        <div className="grid grid-cols-2 gap-4 py-6 mb-6 border-y border-white/10">
            <div>
                <p className="mb-1 text-xs font-bold text-gray-500 uppercase">Due Date</p>
    ...
    <div className={`bg-white/5 border rounded-2xl p-8 ${submission ? 'border-green-500/30' : isOverdue ? 'border-red-500/30' : 'border-white/10'}`}>
        ...
        {submission ? (
            <div className="space-y-6">
                ...
                {submission.grade != null && (
                    <div className="p-6 border bg-blue-500/10 border-blue-500/30 rounded-xl">
                        <h4 className="mb-2 text-lg font-bold text-blue-200">Feedback & Grade</h4>
                        <p className="mb-2 text-3xl font-bold text-white">{submission.grade} <span className="text-sm text-blue-400">/ {assignment.max_points}</span></p>
                        {submission.feedback && <p className="italic text-gray-300">"{submission.feedback}"</p>}
                    </div>
                )}
                ...
                <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                    {submission.file_url && (
                        <div className="p-4 border bg-black/20 rounded-xl border-white/5">
                            <p className="mb-2 text-xs font-bold text-gray-500 uppercase">Attached File</p>
                            <div className="flex items-center justify-between">
                                <div className="flex items-center gap-3">
                                    <File className="w-5 h-5 text-blue-400" />
                                    <span className="text-sm truncate max-w-[150px]">{submission.file_url.split('/').pop()}</span>
                                </div>
                                <a href={`http://localhost:8000${submission.file_url}`} target="_blank" rel="noreferrer" className="p-2 text-blue-400 transition-colors rounded-lg hover:bg-white/10">
                    ...
                    <div className="p-4 border bg-black/20 rounded-xl border-white/5">
                        <p className="mb-2 text-xs font-bold text-gray-500 uppercase">Text Entry</p>
                        <p className="text-sm italic text-gray-300">{submission.text_entry || 'No text provided'}</p>
                    </div>
                </div>

                {!submission.grade && (
                    <button onClick={() => setSubmission(null)} className="text-sm text-gray-500 underline transition-colors hover:text-white">
                        Resubmit Assignment
                    </button>
                )}
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                <div>
                    <label className="block mb-2 text-sm font-medium text-gray-400">Text Content</label>
                    <textarea
                        value={textEntry}
                ...
                <div>
                    <label className="block mb-2 text-sm font-medium text-gray-400">Upload File (PDF, DOC, ZIP, PPTX)</label>
                    <div className="relative group">
                        ...
                        <div className="p-8 text-center transition-colors border-2 border-gray-600 border-dashed rounded-xl group-hover:border-blue-500 bg-black/10">
                            ...
                            <p className="mt-2 text-xs text-gray-600">Maximum file size: 10MB</p>
                        </div>
                    </div>
                </div>
...
const TutorSubmissionView = ({ submissions, assignment, deadline, handleGrade }) => (
    <div className="p-8 border bg-white/5 border-white/10 rounded-2xl">
        <h2 className="mb-6 text-2xl font-bold">Submissions ({submissions.length})</h2>
        <div className="space-y-4">
            {submissions.map(sub => {
                ...
                return (
                    <div key={sub._id} className="p-6 border bg-black/20 rounded-xl border-white/5">
                        <div className="flex items-start justify-between mb-4">
                            <div>
                                ...
                                <p className="mt-1 text-xs text-gray-500">Date: {subDate.toLocaleString()}</p>
                            </div>
                            <div className="text-right">
                                {sub.grade != null ? (
                                    <span className="text-xl font-bold text-green-400">{sub.grade} / {assignment.max_points}</span>
                                ) : (
                                    <span className="px-2 py-1 text-xs italic text-yellow-400 rounded bg-yellow-400/10">Pending Grade</span>
                                )}
                            </div>
                        </div>
                        ...
                        <div className="flex items-end gap-4 p-4 border bg-black/20 rounded-xl border-white/5">
                            <div className="w-32">
                                <label className="block mb-1 text-xs text-gray-500">Score</label>
                                <input
                                    type="number"
                                    placeholder="0"
                            ...
                            <div className="flex-1">
                                <label className="block mb-1 text-xs text-gray-500">Feedback</label>
                                <input
                                    type="text"
...
const AssignmentDetails = () => {
    ...
    if (!assignment) return <div className="py-20 text-center text-white">Assignment not found</div>;
    ...
    return (
        <div className="max-w-4xl p-8 mx-auto text-white">
            <Link to="/assignments" className="inline-block mb-6 text-blue-400 hover:underline">← Back to Assignments</Link>
