...
const getQuizById = async (req, res) => {
  try {
    ...
    const quiz = await Quiz.findById(safeQuizId).populate('course_id', 'title').lean();
    ...
    if (req.user.role === 'Student') {
      ...
      if (!enrollment) {
        return res.status(403).json({
          message: "Not authorized. You must be enrolled in this course to access the quiz."
        });
      }
    }

    // SECURITY FIX: Strip correct answers from response
    const sanitizedQuiz = {
      ...quiz,
...
const createQuiz = async (req, res) => {
  ...
  try {
    ...
    // SECURITY FIX: Authorization check - verify course ownership
    if (
      course.tutor_id.toString() !== safeUserId.toString() &&
      req.user.role !== "Admin"
    ) {
      return res.status(403).json({
        message: "Not authorized to create quiz for this course. You must be the course tutor or admin."
      });
    }
    ...
    // Sanitize questions array with HTML sanitization
    const sanitizedQuestions = questions.map((q) => ({
      ...q,
      question_text: typeof q.question_text === "string"
        ? sanitizeHtml(q.question_text, { allowedTags: [], allowedAttributes: {} }).trim()
        : "",
      options: Array.isArray(q.options)
        ? q.options.map(opt => sanitizeHtml(String(opt), { allowedTags: [], allowedAttributes: {} }).trim())
        : [],
      correct_answer: typeof q.correct_answer === "string"
        ? sanitizeHtml(q.correct_answer, { allowedTags: [], allowedAttributes: {} }).trim()
        : "",
    }));
...
const submitQuiz = async (req, res) => {
  ...
  try {
    ...
    // Get quiz with full details including correct answers
    ...
    if (!enrollment && req.user.role === 'Student') {
      return res.status(403).json({
        message: "Not authorized. You must be enrolled in this course to take the quiz."
      });
    }

    // SECURITY FIX: Server-side grading
    let score = 0;
    quiz.questions.forEach((question, index) => {
      const studentAnswer = answers[index];
      if (studentAnswer !== undefined && studentAnswer === question.correct_option_index) {
        score++;
      }
    });
    ...
    // Return only score information, NOT correct answers
    res.json({
      score,
...
module.exports = { getQuizzes, getQuizById, createQuiz, submitQuiz, getQuizResults };
